...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const votersData = JSON.parse(localStorage.getItem(STORAGE_VOTERS) || '[]');
    const vts = JSON.parse(localStorage.getItem(STORAGE_VOTES) || '[]');

    setPositions(pos);
    setCandidates(cands);
    setVoters(new Set(votersData.map((v: any) => v.id)));
    setVotes(vts);
    ...
    if (positions.length > 0) {
        positions.forEach((pos, idx) => {
          ...
          const chart = new Chart(ctx, {
            type: 'pie',
            data: {
              labels: Object.keys(voteCounts).length ? Object.keys(voteCounts) : ['No votes yet'],
              datasets: [
                {
                  label: 'Votes',
  ...
  return (
    <Container fluid className="p-4">
      ...
      <Row className="mb-5 g-4">
        ...
        <Col md={3} xs={6}>
          <Card className="text-center p-4 shadow-sm">
            <Card.Title className="text-primary">Total Voters</Card.Title>
          ...
          <Carousel.Item key={index}>
            <Card className="p-4 text-center shadow-lg">
              ...
              <div style={{height: '400px'}}>
                <canvas id={`chart-${index}`} role="img" aria-label={`${pos.name} vote chart`}></canvas>
              </div>
            </Card>
          </Carousel.Item>
        ))}
